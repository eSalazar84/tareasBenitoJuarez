...
class RegistroAutomotor{
    todo(array: any):void {
      console.log(`Listado de Autos en el Registro: ${array}`);
    }
    insertar(auto: Auto, array: Auto[]):void {
      if (array.push(auto)) {
        console.log(`Se ha añadido ${auto.brand} a la base de datos ${array}`);
      } else {
        console.log(`El libro ${auto.brand} no se ha podido añadir a la biblioteca`);
      }
    }
    ...
    modificar(brand: string, array: Auto[], dato: string):void {
      ...
      if (autoModificar) {
        ...
        console.log(`El auto ${brand} ha sido modificado y ahora se llama ${dato}`);
      } else {
        console.log("El libro no se ha podido modificar");
      }
    }
    eliminar(brand: string, array: Auto[]): any {
      let autoEncontrado = array.findIndex((auto) => auto.brand == brand);
      if (autoEncontrado >= 0) {
...
const auto_6:Auto = new Auto(`Wolksvagen`,`Gol`,2010);
